...
rm -r notify-server-0.1.0.tar.gz

# Then create the installer folder.
mkdir notify-server-0.1.0 notify-server-0.1.0/logs

# Copy the required dependencies.
cp -R notify-server/target/lib notify-server-0.1.0/
...
chmod a+x notify-server-0.1.0/run.sh
chmod a+x notify-server-0.1.0/stop.sh

# Tar and GZip it for release.
tar -zcvf notify-server-0.1.0.tar.gz notify-server-0.1.0

# Finish
rm -rf notify-server-0.1.0
